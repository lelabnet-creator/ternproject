...
export const agents = pgTable(
  'agents',
  {
    ...
    /**
     * What install this row is, as the agent itself decides.
     *
     * Generated by the agent on its first pairing and kept in its config, so
     * pairing again replaces this row rather than adding a twin beside it.
     * Pairing used to insert unconditionally — nothing in the request could
     * tell "this machine again" from "a new machine", since a hostname, an OS
     * and an architecture are shared by any two machines built from one image.
     *
     * Null for every agent paired before this existed, and for any that still
     * runs an older build. Not unique in SQL for that reason: several nulls are
     * ordinary here, and a unique index would be a constraint on history rather
     * than on what the column means.
     */
    installId: text(),
